...
def process_prediction(pred_path, test_df, output_path):
    ...
    pred_df['candidates'] = pred_df['candidates'].apply(lambda x: x.split(';') if pd.notnull(x) and x != '' else [])
    pred_df[['clue', 'answer', 'answer_length', 'candidates']].to_csv(output_path, index=False)
        ...
        for idx, row in df.iterrows():
            ...
            if pd.notnull(candidates) and candidates != '':
                ...
                if len(cand_list) < 10:
                    ...
                    dummies = [f'dummy{string.ascii_lowercase[len(cand_list) + k]}' for k in range(num_dummies)]
...
def main(test_path, output_dir, constrained_pred, unconstrained_pred):
    ...
    if constrained_pred:
        configs.append({
            ...
            "output_path": os.path.join(output_dir, "final_constrained_predictions.csv")
        })
    
    if unconstrained_pred:
        configs.append({
            "pred_path": unconstrained_pred,
            "output_path": os.path.join(output_dir, "final_unconstrained_predictions.csv")
        })
    ...
    if constrained_pred:
        constrained_output = os.path.join(output_dir, "final_constrained_predictions.csv")
        ...
        insufficient_candidates = df['candidates'].apply(
            lambda x: len(ast.literal_eval(x)) < 10 if pd.notnull(x) and x != '' else True
        )
        problematic_rows = insufficient_candidates[insufficient_candidates].index.tolist()
        if problematic_rows:
            ...
            for pr in problematic_rows:
                print(f"Row {pr}: {df.at[pr, 'clue']}; candidates: '{df.at[pr, 'candidates']}'")
