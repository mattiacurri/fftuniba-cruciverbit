...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Genera candidati per Task 1 in modalità dual (unconstrained/constrained)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--input_file",
        type=str,
        default="hf_evalita2026/task_1/datasets/test_gold.csv",
    ...
    parser.add_argument(
        "--num_beams", type=int, default=100, help="Number of beams for beam search"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="true",
        help="Add special tokens for lengths (true/false)",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def _generate_batch(
    ...
    inputs = tokenizer(
        batch_clues, return_tensors="pt", padding=True, truncation=True, max_length=128
    )
    ...
    for i in range(len(batch_clues)):
        ...
        batch_results.append({"candidates": decoded_candidates, "scores": scores})
...
def print_metrics_report(metrics, mode):
    ...
    # Accuracy@k
    acc_keys = sorted(
        [k for k in metrics.keys() if k.startswith("acc@")],
...
def main():
    ...
    logger.info(
        f"Mode: {'TRAINING (with answers)' if has_answers else 'TESTING (without answers)'}"
    )
    ...
    logger.info(
        f"Parameters: batch_size={args.batch_size}, num_beams={args.num_beams}, num_return_sequences={args.num_return_sequences}"
    )
    ...
    if has_answers:
        metrics_unconstrained = calculate_metrics(
            df_unconstrained, k_values=[1, 5, 10, 100]
        )
    ...
    if has_answers:
        metrics_constrained = calculate_metrics(
            df_constrained, k_values=[1, 5, 10, 100]
        )
