...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate dual-mode Task 2 predictions")
...
def calculate_metrics(targets: List[str], candidates_list: List[List[str]], 
                      k_values: List[int] = [1, 5, 10, 100, 250, 500, 1000]) -> Dict[str, float]:
...
def generate_candidates(model, tokenizer, data: List[Dict], args, device: str) -> Tuple[List[List[str]], List[List[float]]]:
    """
    ...
    for i in tqdm(range(0, total_samples, batch_size), desc="Pass 1"):
        ...
        batch_candidates, batch_scores = _generate_batch(model, tokenizer, batch_data, args, device)
    ...
    if args.max_retries > 0:
        for idx, item in enumerate(data):
            ...
            constrained_count = sum(1 for c in all_candidates[idx] if len(c) == target_len)
            if constrained_count < args.min_candidates:
                retry_indices.append(idx)
    
    if retry_indices:
        LOGGER.info(f"Retry for {len(retry_indices)} clues with few constrained candidates...")
        
        for retry_attempt in range(args.max_retries):
            still_need_retry = []
            
            # Process retries in batches too
            for batch_start in tqdm(range(0, len(retry_indices), batch_size), desc=f"Retry {retry_attempt+1}/{args.max_retries}"):
                ...
                for local_idx, global_idx in enumerate(batch_retry_idx):
                    ...
                    constrained_count = sum(1 for c in all_candidates[global_idx] if len(c) == target_len)
                    if constrained_count < args.min_candidates:
                        still_need_retry.append(global_idx)
            
            retry_indices = still_need_retry
            if not retry_indices:
                LOGGER.info(f"All candidates meet requirements after {retry_attempt+1} retries")
                break
        
        if retry_indices:
            LOGGER.warning(f"{len(retry_indices)} clues still below min_candidates after all retries")
    
    return all_candidates, all_confidences


def _generate_batch(model, tokenizer, batch_data: List[Dict], args, device: str) -> Tuple[List[List[str]], List[List[float]]]:
    """
    Generates candidates for a single batch.
    """
    # Prepare prompts
    prompts = [f"indovinello: {item['clue']} [SL = {item['length']}]" for item in batch_data]
    ...
    for j in range(len(batch_data)):
        ...
        batch_conf = scores[start_idx:end_idx] if len(scores) > 0 else [0.0] * num_return
...
def filter_by_length(candidates: List[str], scores: List[float], target_length: int) -> Tuple[List[str], List[float]]:
...
def save_results(data: List[Dict], all_candidates: List[List[str]], 
                all_confidences: List[List[float]], output_path: str,
    ...
    for i, item in enumerate(data):
        ...
        for idx, cand in enumerate(candidates):
            ...
            if sc:
                ...
                sanitized_confidences.append(confidences[idx] if idx < len(confidences) else 0.0)
        ...
        if filter_length:
            candidates, confidences = filter_by_length(candidates, confidences, target_length)
        ...
        results.append({
            ...
            "clue_num": item['num'],
            "direction": "Horizontal" if item.get('direction', 'A') == 'A' else "Vertical",
            "clue": item['clue'],
...
def print_metrics_report(metrics_unconstrained: Dict, metrics_constrained: Dict, 
                        checkpoint_name: str):
    ...
    # Sort metrics
    keys = sorted(metrics_unconstrained.keys(), 
                 key=lambda k: (0 if 'acc' in k else 1, k))
...
def main():
    ...
    all_candidates, all_confidences = generate_candidates(model, tokenizer, data, args, device)
    ...
    df_unconstrained = save_results(data, all_candidates, all_confidences, 
                                    str(unconstrained_path), filter_length=False)
    
    # Save CONSTRAINED (WITH length filter)
    LOGGER.info("Creating CONSTRAINED CSV (length-filtered)...")
    df_constrained = save_results(data, all_candidates, all_confidences, 
                                  str(constrained_path), filter_length=True)
    ...
    if has_targets:
        targets = [str(item.get('target', item.get('answer', ''))).lower().strip() for item in data]
        ...
        # Print report
        print_metrics_report(metrics_unconstrained, metrics_constrained, checkpoint_name)
